...
public abstract class AbstractRedisAction {
    ...
    protected static <K, V> BString set(K key, V value, RedisDataSource<K, V> redisDataSource) {
    ...
    protected static <K> BString get(K key, RedisDataSource<K, String> redisDataSource) {
    ...
    protected static BInteger append(String key, String value, RedisDataSource<String, String> redisDataSource) {
    ...
    //List Commands
    protected static <K, V> BInteger lPush(K key, RedisDataSource<K, V> redisDataSource, V... value) {
    ...
    protected static <K> BString lPop(K key, RedisDataSource<K, String> redisDataSource) {
    ...
    protected static <K> BMap<K, BString> bLPop(long timeout, RedisDataSource<K, String> redisDataSource, K... keys) {
        RedisListCommands<K, String> redisCommands = null;
        try {
            redisCommands = (RedisListCommands<K, String>) getRedisCommands(redisDataSource);
    ...
    protected static <K> BMap<K, BString> bRPop(long timeout, RedisDataSource<K, String> redisDataSource, K... keys) {
        RedisListCommands<K, String> redisCommands = null;
        try {
            redisCommands = (RedisListCommands<K, String>) getRedisCommands(redisDataSource);
    ...
    protected static <K> BString lIndex(K key, long index, RedisDataSource<K, String> redisDataSource) {
        RedisListCommands<K, String> redisCommands = null;
        try {
            redisCommands = (RedisListCommands<K, String>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BInteger lLen(K key, RedisDataSource<K, V> redisDataSource) {
        RedisListCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisListCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K> BArray lRange(K key, long start, long stop, RedisDataSource<K, String> redisDataSource) {
        RedisListCommands<K, String> redisCommands = null;
        try {
            redisCommands = (RedisListCommands<K, String>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BInteger lRem(K key, long count, V value, RedisDataSource<K, V> redisDataSource) {
        RedisListCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisListCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BString lSet(K key, long index, V value, RedisDataSource<K, V> redisDataSource) {
        RedisListCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisListCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BString lTrim(K key, long start, long stop, RedisDataSource<K, V> redisDataSource) {
        RedisListCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisListCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K> BString rPop(K key, RedisDataSource<K, String> redisDataSource) {
        RedisListCommands<K, String> redisCommands = null;
        try {
            redisCommands = (RedisListCommands<K, String>) getRedisCommands(redisDataSource);
    ...
    protected static <K> BString rPopLPush(K src, K destination, RedisDataSource<K, String> redisDataSource) {
        RedisListCommands<K, String> redisCommands = null;
        try {
            redisCommands = (RedisListCommands<K, String>) getRedisCommands(redisDataSource);
            String result = redisCommands.rpoplpush(src, destination);
            return new BString(result);
        } catch (IllegalArgumentException e) {
            throw new BallerinaException(ARGUMENTS_MUST_NOT_BE_NULL);
        } finally {
    ...
    protected static <K, V> BInteger rPush(K key, RedisDataSource<K, V> redisDataSource, V... values) {
        RedisListCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisListCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BInteger zAdd(K key, RedisDataSource<K, V> redisDataSource, Map<V, Double> valueScoreMap) {
    ...
    protected static <K, V> BInteger zCard(K key, RedisDataSource<K, V> redisDataSource) {
        RedisSortedSetCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisSortedSetCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BInteger zCount(K key, double min, double max, RedisDataSource<K, V> redisDataSource) {
    ...
    protected static <K, V> BFloat zIncrBy(K key, double amount, V member, RedisDataSource<K, V> redisDataSource) {
        RedisSortedSetCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisSortedSetCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BInteger zInterStore(K dest, RedisDataSource<K, V> redisDataSource, K... keys) {
        RedisSortedSetCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisSortedSetCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BInteger zLexCount(K key, V min, V max, RedisDataSource<K, V> redisDataSource) {
        RedisSortedSetCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisSortedSetCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K> BArray zRange(K key, long min, long max, RedisDataSource<K, String> redisDataSource) {
    ...
    protected static <K> BArray zRangeByLex(K key, String min, String max, RedisDataSource<K, String> redisDataSource) {
        RedisSortedSetCommands<K, String> redisCommands = null;
        try {
            redisCommands = (RedisSortedSetCommands<K, String>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BInteger zRank(K key, V member, RedisDataSource<K, V> redisDataSource) {
        RedisSortedSetCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisSortedSetCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BInteger zRem(K key, RedisDataSource<K, V> redisDataSource, V... members) {
        RedisSortedSetCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisSortedSetCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K> BArray zRevRange(K key, long min, long max, RedisDataSource<K, String> redisDataSource) {
        RedisSortedSetCommands<K, String> redisCommands = null;
        try {
            redisCommands = (RedisSortedSetCommands<K, String>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BInteger zRevRank(K key, V member, RedisDataSource<K, V> redisDataSource) {
        RedisSortedSetCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisSortedSetCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BFloat zScore(K key, V member, RedisDataSource<K, V> redisDataSource) {
        RedisSortedSetCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisSortedSetCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BInteger zUnionStore(K dest, RedisDataSource<K, V> redisDataSource, K... keys) {
        RedisSortedSetCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisSortedSetCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BInteger hDel(K key, RedisDataSource<K, V> redisDataSource, K... fields) {
    ...
    protected static <K, V> BBoolean hExists(K key, K field, RedisDataSource<K, V> redisDataSource) {
        RedisHashCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisHashCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K> BString hGet(K key, K field, RedisDataSource<K, String> redisDataSource) {
    ...
    protected static <K> BMap<K, BString> hGetAll(K key, RedisDataSource<K, String> redisDataSource) {
        RedisHashCommands<K, String> redisCommands = null;
        try {
            redisCommands = (RedisHashCommands<K, String>) getRedisCommands(redisDataSource);
    ...
    protected static <K> BInteger hIncrBy(K key, K field, long amount, RedisDataSource<K, String> redisDataSource) {
        RedisHashCommands<K, String> redisCommands = null;
        try {
            redisCommands = (RedisHashCommands<K, String>) getRedisCommands(redisDataSource);
    ...
    protected static BArray hKeys(String key, RedisDataSource<String, String> redisDataSource) {
    ...
    protected static <K> BInteger hLen(K key, RedisDataSource<K, String> redisDataSource) {
        RedisHashCommands<K, String> redisCommands = null;
        try {
            redisCommands = (RedisHashCommands<K, String>) getRedisCommands(redisDataSource);
    ...
    protected static <K> BMap<K, BString> hMGet(K key, RedisDataSource<K, String> redisDataSource, K... fields) {
        RedisHashCommands<K, String> redisCommands = null;
        try {
            redisCommands = (RedisHashCommands<K, String>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BString hMSet(K key, Map<K, V> fieldValueMap, RedisDataSource<K, V> redisDataSource) {
        RedisHashCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisHashCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BBoolean hSet(K key, K field, V value, RedisDataSource<K, V> redisDataSource) {
        RedisHashCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisHashCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BBoolean hSetNx(K key, K field, V value, RedisDataSource<K, V> redisDataSource) {
        RedisHashCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisHashCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BInteger hStrln(K key, K field, RedisDataSource<K, V> redisDataSource) {
        RedisHashCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisHashCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K> BArray hVals(K key, RedisDataSource<K, String> redisDataSource) {
        RedisHashCommands<K, String> redisCommands = null;
        try {
            redisCommands = (RedisHashCommands<K, String>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BInteger exists(RedisDataSource<K, V> redisDataSource, K... keys) {
        RedisKeyCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisKeyCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BBoolean expire(K key, long seconds, RedisDataSource<K, V> redisDataSource) {
        RedisKeyCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisKeyCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <V> BArray keys(String pattern, RedisDataSource<String, V> redisDataSource) {
    ...
    protected static <K, V> BBoolean move(K key, int db, RedisDataSource<K, V> redisDataSource) {
        RedisKeyCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisKeyCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BBoolean persist(K key, RedisDataSource<K, V> redisDataSource) {
        RedisKeyCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisKeyCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BBoolean pExpire(K key, long milliSeconds, RedisDataSource<K, V> redisDataSource) {
        RedisKeyCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisKeyCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BInteger pTtl(K key, RedisDataSource<K, V> redisDataSource) {
        RedisKeyCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisKeyCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K> BString randomKey(RedisDataSource<K, String> redisDataSource) {
    ...
    protected static <K> BString rename(K key, K newName, RedisDataSource<K, String> redisDataSource) {
        RedisKeyCommands<K, String> redisCommands = null;
        try {
            redisCommands = (RedisKeyCommands<K, String>) getRedisCommands(redisDataSource);
    ...
    protected static <K> BBoolean renameNx(K key, K newName, RedisDataSource<K, String> redisDataSource) {
        RedisKeyCommands<K, String> redisCommands = null;
        try {
            redisCommands = (RedisKeyCommands<K, String>) getRedisCommands(redisDataSource);
    ...
    protected static <K> BArray sort(K key, RedisDataSource<K, String> redisDataSource) {
        RedisKeyCommands<K, String> redisCommands = null;
        try {
            redisCommands = (RedisKeyCommands<K, String>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BInteger ttl(K key, RedisDataSource<K, V> redisDataSource) {
        RedisKeyCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisKeyCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BString type(K key, RedisDataSource<K, V> redisDataSource) {
        RedisKeyCommands<K, V> redisCommands = null;
        try {
            redisCommands = (RedisKeyCommands<K, V>) getRedisCommands(redisDataSource);
    ...
    protected static <K, V> BString auth(String password, RedisDataSource<K, V> redisDataSource) {
    ...
    protected static <K> BString echo(String message, RedisDataSource<K, String> redisDataSource) {
    ...
    protected static <K> BString ping(RedisDataSource<K, String> redisDataSource) {
        RedisCommands<K, String> redisCommands = null;
        try {
            redisCommands = (RedisCommands<K, String>) getRedisCommands(redisDataSource);
    ...
    private static BArray createBStringArrayFromList(List<String> list) {
        BArray bStringArray = new ArrayValueImpl(new String[list.size()]);
        int i = 0;
    ...
    protected static String[] createStringArrayFromBArray(BArray bStringArray) {
    ...
    protected static Map<String, Object> createMapFromBMap(BMap<String, BString> bMap) {
        Map<String, Object> map = new HashMap<>();
        String[] keys = bMap.getKeys();
        for (int i = 0; i < keys.length; i++) {
            map.put(keys[i], bMap.get(keys[i]));
        }
        return map;
    }

    protected static BArray createBstringArrayFromBMap(BMap<String, BString> bMap) {
        String[] keys = bMap.getKeys();
        BArray bStringArray = new ArrayValueImpl(new String[bMap.size()]);
        for (int i = 0; i < keys.length; i++) {
            bStringArray.add(i, bMap.get(keys[i]).stringValue());
            i++;
        }
        return bStringArray;
    }
