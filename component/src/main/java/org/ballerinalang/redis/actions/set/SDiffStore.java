...
/**
 * {@code {@link SDiffStore}} Maps with "SDIFFSTORE" operation of Redis.
 *
 * @since 0.5.0
 */
@BallerinaFunction(orgName = "wso2",
                   packageName = "redis:0.0.0",
                   functionName = "sDiffStore",
                   receiver = @Receiver(type = TypeKind.OBJECT,
                                        structType = Constants.CALLER_ACTIONS))
public class SDiffStore extends AbstractRedisAction {
    ...
    public void execute(Context context) {
        ...
        BInteger result = sDiffStore(dest, redisDataSource, createArrayFromBStringArray(keys));
        try {
