...
/**
 * {@code {@link ZInterStore}} Maps with "ZINTERSTORE" operation of Redis.
 *
 * @since 0.5.0
 */
@BallerinaFunction(orgName = "wso2",
                   packageName = "redis:0.0.0",
                   functionName = "zInterStore",
                   receiver = @Receiver(type = TypeKind.OBJECT,
                                        structType = Constants.CALLER_ACTIONS))
public class ZInterStore extends AbstractRedisAction {
    ...
    public void execute(Context context) {
        ...
        BInteger result = zInterStore(dest, redisDataSource, createArrayFromBStringArray(keys));
        try {
