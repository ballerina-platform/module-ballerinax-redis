...
/**
 * {@code {@link SIsMember}} Maps with "SISMEMBER" operation of Redis.
 *
 * @since 0.5.0
 */
@BallerinaFunction(orgName = "wso2",
                   packageName = "redis:0.0.0",
                   functionName = "sIsMember",
                   receiver = @Receiver(type = TypeKind.OBJECT,
                                        structType = Constants.CALLER_ACTIONS))
public class SIsMember extends AbstractRedisAction {
    ...
    public void execute(Context context) {
        ...
        BBoolean result = sIsMember(key, value, redisDataSource);
        try {
