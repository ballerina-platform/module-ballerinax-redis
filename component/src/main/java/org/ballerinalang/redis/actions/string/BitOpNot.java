...
/**
 * {@code BitOpNot} Maps with "bitopnot" operation of Redis.
 *
 * @since 0.5.0
 */
@BallerinaFunction(orgName = "wso2",
                   packageName = "redis:0.0.0",
                   functionName = "bitOpNot",
                   receiver = @Receiver(type = TypeKind.OBJECT,
                                        structType = Constants.CALLER_ACTIONS))
public class BitOpNot extends AbstractRedisAction {
    ...
    public void execute(Context context) {
        ...
        String key = context.getStringArgument(1);
        BInteger result = bitopNot(destination, key, redisDataSource);
        try {
