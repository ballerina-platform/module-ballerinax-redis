...
/**
 * {@code {@link SMembers}} Maps with "SMEMBERS" operation of Redis.
 *
 * @since 0.5.0
 */
@BallerinaFunction(orgName = "wso2",
                   packageName = "redis:0.0.0",
                   functionName = "sMembers",
                   receiver = @Receiver(type = TypeKind.OBJECT,
                                        structType = Constants.CALLER_ACTIONS))
public class SMembers extends AbstractRedisAction {
    ...
    public void execute(Context context) {
        ...
        BStringArray result = sMembers(key, redisDataSource);
        try {
