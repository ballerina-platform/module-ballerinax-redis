...
/**
 * {@code rPopLPush} Maps with "RPOPLPUSH" operation of Redis.
 *
 * @since 0.5.0
 */
@BallerinaAction(packageName = "ballerina.data.redis",
                 actionName = "rPopLPush",
                 connectorName = Constants.CONNECTOR_NAME,
                 args = {
                         @Argument(name = "c",
                                   type = TypeKind.CONNECTOR)
                 })
public class RPopLPush extends AbstractRedisAction {
    ...
    public ConnectorFuture execute(Context context) {
        ...
        String destination = getStringArgument(context, 1);
        BString result = rPopLPush(source, destination, redisDataSource);
