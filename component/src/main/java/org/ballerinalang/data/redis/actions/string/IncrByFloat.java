...
/**
 * {@code incrByFloat} Maps with "INCRBYFLOAT" operation of Redis.
 *
 * @since 0.5.0
 */
@BallerinaAction(packageName = "ballerina.data.redis",
                 actionName = "incrByFloat",
                 connectorName = Constants.CONNECTOR_NAME,
                 args = {
                         @Argument(name = "c",
                                   type = TypeKind.CONNECTOR)
                 })
public class IncrByFloat extends AbstractRedisAction {
    ...
    public ConnectorFuture execute(Context context) {
        ...
        double value = getDoubleArgument(context, 0);
