...
/**
 * {@code {@link ZRemRangeByLex}} Maps with "ZREMRANGEBYLEX" operation of Redis.
 *
 * @since 0.5.0
 */
@BallerinaAction(packageName = "ballerina.data.redis",
                 actionName = "zRemRangeByLex",
                 connectorName = Constants.CONNECTOR_NAME,
                 args = {
                         @Argument(name = "c",
                                   type = TypeKind.CONNECTOR)
                 })
public class ZRemRangeByLex extends AbstractRedisAction {
    ...
    public ConnectorFuture execute(Context context) {
        ...
        BInteger result = zRemRangeByLex(key, min, max, redisDataSource);
