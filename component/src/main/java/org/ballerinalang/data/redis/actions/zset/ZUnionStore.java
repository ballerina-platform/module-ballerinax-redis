...
/**
 * {@code {@link ZUnionStore}} Maps with "ZUNIONSTORE" operation of Redis.
 *
 * @since 0.5.0
 */
@BallerinaAction(packageName = "ballerina.data.redis",
                 actionName = "zUnionStore",
...
public class ZUnionStore extends AbstractRedisAction {
    ...
    public ConnectorFuture execute(Context context) {
        ...
        BStringArray members = (BStringArray) getRefArgument(context, 1);
        if (members == null) {
            throw new BallerinaException("Member array " + MUST_NOT_BE_NULL);
        }
        BInteger result = zUnionStore(dest, redisDataSource, createArrayFromBStringArray(members));
