...
/**
 * {@code {@link ZRevRangeByLex}} Maps with "ZREVRANGEBYLEX" operation of Redis.
 *
 * @since 0.5.0
 */
@BallerinaAction(packageName = "ballerina.data.redis",
                 actionName = "zRevRangeByLex",
                 connectorName = Constants.CONNECTOR_NAME,
                 args = {
                         @Argument(name = "c",
                                   type = TypeKind.CONNECTOR)
                 })
public class ZRevRangeByLex extends AbstractRedisAction {
    ...
    public ConnectorFuture execute(Context context) {
        ...
        String min = getStringArgument(context, 1);
        String max = getStringArgument(context, 2);
        BStringArray result = zRevRangeByLex(key, min, max, redisDataSource);
