...
/**
 * {@code {@link HIncrByFloat}} Maps with "HINCRBYFLOAT" operation of Redis.
 *
 * @since 0.5.0
 */
@BallerinaAction(packageName = "ballerina.data.redis",
                 actionName = "hIncrByFloat",
                 connectorName = Constants.CONNECTOR_NAME,
                 args = {
                         @Argument(name = "c",
                                   type = TypeKind.CONNECTOR)
                 })
public class HIncrByFloat extends AbstractRedisAction {
    ...
    public ConnectorFuture execute(Context context) {
        ...
        String field = getStringArgument(context, 1);
        double amount = getDoubleArgument(context, 0);
        BFloat result = hIncrByFloat(key, field, amount, redisDataSource);
