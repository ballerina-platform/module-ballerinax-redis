...
/**
 * {@code {@link ZRemRangeByScore}} Maps with "ZREMRANGEBYSCORE" operation of Redis.
 *
 * @since 0.5.0
 */
@BallerinaAction(packageName = "ballerina.data.redis",
                 actionName = "zRemRangeByScore",
                 connectorName = Constants.CONNECTOR_NAME,
                 args = {
                         @Argument(name = "c",
                                   type = TypeKind.CONNECTOR)
                 })
public class ZRemRangeByScore extends AbstractRedisAction {
    ...
    public ConnectorFuture execute(Context context) {
        ...
        double min = getDoubleArgument(context, 0);
        double max = getDoubleArgument(context, 1);
        BInteger result = zRemRangeByScore(key, min, max, redisDataSource);
